...
impl<T> Timer<T> {
    ...
    fn poll_to(&mut self, mut target_tick: Tick) -> Option<T> {
        ...
        if let Some(_inner) = self.inner.borrow() {
    ...
    fn schedule_readiness(&self, tick: Tick) {
        if let Some(inner) = self.inner.borrow() {
            ...
            loop {
                ...
                let actual = inner
                    .wakeup_state
                    .compare_exchange(curr, tick as usize, Ordering::Release, Ordering::Relaxed)
                    .unwrap_or_else(|x| x);
