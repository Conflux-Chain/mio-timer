...
use crate::timer::{Builder as TimerBuilder, Timeout, Timer};
use log::{debug, trace};
use mio::{
    Events, Interest, Poll, Token, Waker,
    event::{Event, Source},
};
use mio_misc::{NotificationId, channel, queue::NotificationQueue};
use std::{
    default::Default,
    fmt, io,
    sync::{Arc, mpsc},
...
impl<H: Handler> EventLoop<H> {
    ...
    pub fn timeout(&mut self, token: H::TimeoutState, delay: Duration) -> Timeout {
    ...
    fn io_process(&mut self, handler: &mut H) {
        ...
        let events: Vec<Event> = self.events.iter().map(|e| e.clone()).collect();

        debug!(
            "event loop got {} events, queue len {}",
            events.len(),
            self.notify_queue.len()
        );

        for evt in events {
            debug!("event loop processing event: {:?}", evt);
            match evt.token() {
                NOTIFY_OR_TIMER => {
                    while self.notify_queue.len() > 0 {
                        match self.notify_queue.pop() {
                            ...
                            None => {
                                ...
                                trace!("event loop got spurious wakeup with no id");
                            }
                        }
                    }
                }
